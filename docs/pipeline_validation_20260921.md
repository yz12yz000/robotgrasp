...
3. 原准备检查只确认收到了数据，可能将冻结数据误认为就绪。
   现在要求两组新鲜、时间戳更新、1280×800、坐标系相同且配对差不超过 100 ms 的数据。
...
已建立固定桌面模型并接入 Planning Scene，详见 [IMPLEMENT_pengzhuang.md](../IMPLEMENT_pengzhuang.md)。当前现场预览已能完成 YOLO 和 rim locator；单目标 XYZ 点云按 object_id=0 兼容处理。最近一次只规划预览在 `approach_ik_failed:-31` 处拒绝，未发送机械臂动作；这表示当时机械臂当前姿态与固定抓取姿态没有可用 IK，需要调整姿态或目标，不应绕过检查。
