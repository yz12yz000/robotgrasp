...
8. 已根据现场核对确认两指开合方向是 `grasp_center/tool0` 的 Y 轴，因此将壁面外法向作为工具 Y（闭合）轴；工具 Z 保持向下作为现有竖直接近方向，工具 X 用右手系补齐。斜壁时允许工具 Z 随 Y 轴做最小正交倾斜。若更换夹爪或 URDF，必须重新核对轴向，不能只凭 frame 名称假设。
...
当前实现采用如下几何构造：`c=向外壁面单位法向`（工具 Y，即闭合轴），`d=(0,0,-1)`，`x=normalize(c×d)`，`z=normalize(x×c)`，最终 `R_tool=[x,c,z]`。这样竖直壁时工具 Z 向下，斜壁时仍保持右手系并产生对应 RPY；当叉积长度过小、法向不稳定或倾角超出已验收范围时拒绝。
...
## 12. 本次调试与实施结果（2026-09-22）

本次已按上述逻辑检查并接入现有程序：

- 固定参考保存于 `src/grasp_executor/config/place_config.json` 的 `place_reference_*`；执行时不读取当前机械臂位置覆盖配置。
- `MoveItArm.resolve_place_pose()` 将记录的 `base/tool0` 参考通过 `base_link <- base` 和 `tool0 <- grasp_center` 静态 TF 转换为执行器使用的 `base_link/grasp_center`，并与保存的 `place_ready_*` 交叉核对；不一致时拒绝执行。
- 每个目标按接近、下降、抬升、转移到准备位、下降 20 cm、退回准备位共六段规划；放置点到位并静止后独立保持 1 秒，再发送 Open。
- 单目标 XYZ 点云缺少 `object_id` 时按单对象 0 处理，多实例点云仍使用 UINT32 `object_id`。
- 保留桌面碰撞模型和夹爪保护盒；更新 Planning Scene 时保留原有 SRDF 自碰撞矩阵。

已通过：

```text
纯 Python 回归测试（grasp_executor、rim_locator、yolo_vision、table_model）：54 passed
隔离 MoveIt 测试：5 项通过，无真实硬件动作
place reference 转换测试：通过
New_run 脚本 bash -n：通过
```

现场只规划预览已完成 YOLO 和 rim locator；当前机械臂姿态下固定抓取姿态曾返回 `approach_ik_failed:-31`，因此没有发送真实运动。该问题属于当前抓取目标/机械臂姿态的 IK 可达性，不能通过重新读取当前位置覆盖固定放置参考来掩盖。真实执行前必须先让 `run_pipeline_place_test.sh` 完整返回六段规划通过。

统一启动入口和顺序已放在 `New_run/README.md`。其中 `New_run/*.sh` 是根目录脚本的单一入口包装，不复制维护第二份实现，避免两个目录的脚本逻辑漂移。
